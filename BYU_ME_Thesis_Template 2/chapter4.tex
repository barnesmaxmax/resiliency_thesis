...
\section{Overview}
In this chapter, Resiliency Model is applied to evaluate the effects of link
loss or degradation on the USTM network. To do this, the model is applied to scenarios
...
After identifying locations of interest, the model is now applied each of the scenarios to compare the results of the 40 selected scenarios. In each scenario, an individual highway
...
Overall, two key findings are drawn from this analysis:
\begin{enumerate}
  \item Cost estimates are different by trip purpose and overall depending on
  which estimation method is used
  \item The majority of cost is generated by Freight trips
\end{enumerate}
...
just those that are included in the Resiliency Model.
...
link that was cut. Additionally, a few scenarios returned positive benefits, which is not entirely intuitive. These resutls will be discussed in greater depth later in this chapter. A ranking is provided for all of the scenarios examined
