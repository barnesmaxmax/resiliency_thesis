...
destination based on its size or level of attraction (e.g., in an ideal
...
could be created. The model framework used to create the presented model
...
understanding of the distance between zones by multiple modes, and how
...
is an important mode to include in the model, and  should be
...
The presented model also uses socioeconomic data to estimate the productions at
...
the model, where $\alpha$ denotes mode choice constants, $\beta$ denotes
...
variable in the presented model. Second, both the $AUTO$ and $NMOT$ equations account for a
...
To ensure the model is accurately reflecting the distribution
