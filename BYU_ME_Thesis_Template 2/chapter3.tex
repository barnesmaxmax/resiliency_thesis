...
Next, the logsum calculation is used. The logsum is similar to the denominator of the
probability function, however the logsum captures the total value of the choice set, and is interpreted as the
...
in the presented model include Home-based work (HBW), Home-based other (HBO),
and non-home based (NHB) trips are primarily considered. Other trips purposes, such as recreation (REC) trips and
...
\noindent Finding the difference in travel time for each scenario allows for
...
Table \ref{tab:VOT}. The method for deriving this value is shown in Equation \ref{eq:totalcost}.
...
single user for each purpose, Equation \ref{eq:ttmethod} must be used.
