...
sensitivity compared with gravity and four-step models, and are advantageous because
...
\begin{equation}
\mathcal{P}_{ij} = \frac{e^{u_{ij}}}{\sum_{j\in J}e^{u_{k}}}
  \label{eqn:probability}
\end{equation}

\noindent Where $\mathcal{P_{ij}}$ represents the probability of trips made between an
...
\begin{equation}
 Logsum = \ln\sum\exp(f(\beta_i, C_{ij}, A_j, \gamma_i, t_{ij}))
  \label{eqn:logsum}
\end{equation}

\noindent where $\beta_i$ represents a mode choice coefficient, $C_{ij}$ represents
a data variable, $A_j$ represents the attraction at zone $j$, $\gamma_i$ represents
a DC parameter, and $t_{ij}$ represents the travel time between an OD pair.

The ability to incorporate multiple types of data, as in Equation \ref{equn:logsum}, as well as improved methods
...
\begin{equation}
\mathcal{P}_{ijk}(m) = \frac{\exp(f(\beta_m * t_{ijk}))}
{\sum_{K}\exp(f(\beta * t_{ijk}))}
  \label{eq:mcp}
\end{equation}
...
\begin{equation}
\mathcal{P}_{ij}(m) = \frac{\exp(f(\gamma_m, MCLS_{ijm}, A_j))}
