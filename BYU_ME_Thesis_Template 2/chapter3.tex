...
\begin{equation}
{P}_{ij} = \frac{e^{u_{ij}}}{\sum_{j\in J}e^{u_{k}}}
  \label{eqn:probability}
\end{equation}

\noindent Where $P_{ij}$ represents the probability of trips made between an
origin $i$ and a destination $j$ among all destinations $J$. The probabilities
...
we have the logsum calculation,

\begin{equation}
 Logsum = \ln\sum\exp(f(\beta_i, , C_i, A_j, \gamma_i, t_{ij}))
  \label{eqn:logsum}
\end{equation}

\noindent The ability to incorporate multiple types of data and improved methods
...
in Equation \ref{eqn:tripsijk}, such as accessibility by automobile,
...
A critical feature of logit-based choice models – described briefly in Chapter
\ref{chp:chapter2} – is that they are more versatile than traditional modeling
...
\begin{equation}
{P}_{ijk}(m) = \frac{\exp(f(\beta \times t_{ijk}))}
{\sum_{K}\exp(f(\beta \times t_{ijk}))}
  \label{eq:mcp}
\end{equation}

\noindent The log of the denominator of the this equation is called the
mode choice logsum, \(MCLS_{ijk}\) and is a measure of the travel cost by
all modes, $k$, weighted by utility parameters \(\beta\) that may vary by
...
\begin{equation}
{P}_{ij}(m) = \frac{\exp(f(\gamma, MCLS_{ijk}, A_j))}
{\sum_{J}\exp(f(\gamma, MCLS_{ijk}, A_j))}
  \label{eq:dcp}
\end{equation}
...
\begin{equation}
\Delta = DCLS^{\mathrm{Base}} - DCLS^{\mathrm{Scenario}}
  \label{eq:deltas}
\end{equation}
...
trip},
meaning that the total lost accessibility is \(P_{ik} * \Delta_{DCLS}\) where
\(P\) is
...
\begin{equation}
\mathrm{Cost} = \sum_{I}\sum_{K} -1 / \beta_{\mathrm{cost},k} * P_{ik}
\Delta_{ik}
  \label{eq:totalcost}
\end{equation}
...
}{\beta_{\mathrm{cost}, k}} T_{ijk} t_{ijk}
  \label{eq:ttmethod}
\end{equation}
...
Of particular interest from the available information was the $AM_{TIME}$ which
...
The $AM_{TIME}$ was used to determine the travel time between an origin and
...
into the model. We used the $AM_{TIME}$ and $DISTANCE$ variables available in
...
The NMOT trip distances are held fixed, similarly to transit distances. We
...
trips constant was made for several reasons, mainly because pedestrians often
cut through construction zones, or can find shorter detours on side streets
that may not be viable for large amounts of traffic.
...
, or adapted from SWIM. The Travel Time represented with $TT_ij$, Travel Cost represented as $C_{Cost}$, Walk Distance (less than 1 mile) represented as $D_{<1MILE}$, and Walk Distance (1 mile or more) represented as $D_{>1MILE}$ are the coefficients for the
...
The following are sample utility equations for each of the modes considered in the Resiliency Model. $\alpha$ denotes mode choice constants, $\beta$ denotes mode choice coefficients, and $\gamma$ denotes destination choice parameters. Additionally, $C$ will denote other variables which are typically zonally related.

\begin{equation}
U_{{AUTO}_{ij}} = \beta1_{ij} \times C1_{ij} + \beta2_{ij} \times C2_{ij}
	\label{eqn:auto}
\end{equation}

\begin{equation}
U_{{TRANSIT}_{ij}} = \alpha_{TRN} + \beta1_{ij} \times C1_{ij} + \beta2_{ij} \times C2_{ij}
	\label{eqn:transit}
\end{equation}

\begin{equation}
U_{{NMOT}_{ij}} = \alpha_{NMOT} + 20 * (\beta1_{ij} \times C1_{ij} + \beta2_{ij} \times C2_{ij})
	\label{eqn:nmot}
\end{equation}
...
\begin{equation}
	Probability_{ijk} = \frac{\exp U_{ij}}{\sum \exp U_{ijk}}
	\label{eqn:prob}
\end{equation}
...
The DC equation, seen in Equation \ref{eqn:dc}, is made up of the
...
\begin{equation}
\begin{aligned}
	U_{ij} = \gamma1 \times MCLS_{ij} + \log (SIZETERM_{j}) + \gamma2 \times C2_{D}\\ + \gamma3_{ij} \times C3_{D^2}
   + \gamma4 _{ij} \times D^3_{ij}
\label{eqn:dc}
\end{aligned}
\end{equation}
...
\begin{equation}
\begin{aligned}
	SIZETERM_i = \eta1_{j} \times C_1 + \eta2_{j} \times C_2 + \eta3_{j} \times C_{3}
	\label{sizeterm}
\end{aligned}
\end{equation}

\noindent Where each $C$ represents a different zonal statistic in the UHTS data,
and $eta$ represents coefficients used.
...
\begin{equation}
	Trips_{ij} = P_i * DCPROBABILITY_{ij}
	\label{eqn:ij}
\end{equation}
...
Additionally, Figure \ref{fig:ustm_tlfd} shows the original versus the final
